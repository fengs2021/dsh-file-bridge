...
export function apply(ctx, config) {
  ...
  // ──────────────────────────── 1) /files 通道：客户端上传 RPC ────────────────────────────
  // 客户端通过 connection.rpc.call("/files", "upload", { sessionId, name, mediaType, dataBase64 })
  // 把非图片文件投到共享目录并拿到可访问 URL。DSH 官方 attachment 只支持图片，
  // 这里补全"非图片附件上传"的 RPC 通道（host 端 + client 端配套使用）。
  // 文件大小上限：默认 20MB（base64 前），防 DoS / nginx client_max_body_size 25m。
  const MAX_UPLOAD_BYTES = 20 * 1024 * 1024;
  const ALLOWED_PREFIX = "/dsh-files/";

  ctx.effect(() => {
    return connection.rpc.handle(
      "/files",
      async (endpoint, payload, signal) => {
        // 兜底信号中止检查（connection 已经会传 AbortSignal；这里仅防御未传的情况）
        ...
        try {
          ...
          return {
            ok: false,
            error: { code: "bad-request", message: "base64 解码失败", details: { issues: [] } },
          };
        }
        if (bytes.byteLength === 0) {
          return {
            ok: false,
            error: { code: "bad-request", message: "空文件", details: { issues: [] } },
          };
        }
        if (bytes.byteLength > MAX_UPLOAD_BYTES) {
          return {
            ...
            error: {
              code: "bad-request",
              message: `文件过大（${bytes.byteLength} > ${MAX_UPLOAD_BYTES}）`,
              details: { issues: [] },
            },
          };
        }
        // 落盘：用 uniqueName 防同名覆盖
        ...
        try {
          ...
          return {
            ok: false,
            error: { code: "internal", message: `写入失败: ${err && err.message}`, details: {} },
          };
        }
        const url = publicBase(config) + ALLOWED_PREFIX + encodeURIComponent(fileName);
        return {
          ok: true,
          value: {
            name: fileName,
      ...
      { authority: "trusted-host" } // 上传经过 nginx + Basic Auth,host 信任
    );
  }, "file-bridge: /files upload rpc");

  // ──────────────────────────── 2) 静态文件服务：/dsh-files/* 只读伺服 ────────────────────────────
  // 客户端拿到 URL（如 https://host/dsh-files/foo.pdf）后浏览器 GET 拉取 → nginx 反代到
  // 3081 → 这里。前端 SPA fallback seat 对非 GET/HEAD 直接 405,而我们的 prefix route 是
  // GET 友好,优先于 fallback。注意：路径 /dsh-files/<name> 中的 name 是 encodeURIComponent 过的
  // 中文文件名，需要 decodeURIComponent 反向还原,再用 statSync 防穿越。
  ctx.effect(() => {
    return webServer.register({
      ...
      handler: async (req, res) => {
        try {
          ...
          const tail = rawPath.slice("/dsh-files".length).replace(/^\/+/, "");
          ...
          // 二次防御: decode 后还含路径分隔符/.. → 拒绝
          const resolved = resolve(join(dir, basename(decoded)));
          ...
          const mime = mimeFor(decoded);
          // 中文文件名走 RFC 5987
          const encodedName = encodeURIComponent(decoded);
          const asciiFallback = decoded.replace(/[^\x20-\x7E]/g, "_");
          res.writeHead(200, {
            "content-type": mime,
