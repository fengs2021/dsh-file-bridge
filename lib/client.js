...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     * ⚠️ 长截图保护：早期实现只按「最长边 ≤1600」压缩，对手机长截图（如 810×14400）
     * 会把宽度压到 810×(1600/14400)≈91px，文字全糊、VLM 无法识别（症状：图片能上传，
     * 但「识别不对/看不清」）。修复：
     *  - 宽高比极端的图（>2.5）放宽最长边上限到 4096，给文字留宽度；
     *  - 保底最短边 ≥ 360px，保证文字可读（最长边因此可能略超上限，可读性优先）。
     */
    function compressImage(file) {
      return new Promise((resolve, reject) => {
        ...
        img.onload = () => {
          try {
            const MAX = 1600; // 普通图最长边
            const LONG_EDGE_MAX = 4096; // 长截图最长边上限（放宽）
            const MIN_EDGE = 360; // 最短边下限，保证文字可读
            let w = img.naturalWidth || img.width;
            let h = img.naturalHeight || img.height;
            if (w <= 0 || h <= 0) throw new Error("bad image");
            const aspect = Math.max(w, h) / Math.min(w, h);
            const maxEdge = aspect > 2.5 ? LONG_EDGE_MAX : MAX;
            let scale = Math.min(1, maxEdge / Math.max(w, h));
            // 保底最短边可读
            const minSide = Math.min(w, h);
            const needScale = MIN_EDGE / minSide;
            if (needScale > scale && needScale <= 1) {
              scale = needScale; // 允许略超 maxEdge，文字可读性优先
            }
